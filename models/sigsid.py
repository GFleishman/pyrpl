# -*- coding: utf-8 -*-
"""
The symmetric interleaved geodesic shooting in diffeomorphisms model.
...
# TODO: consider some kind of bootstrapping for symmetry
# there seems to only be deformation where both images have a strong gradient
# i.e. the intersection of gradients above some threshold
# would be better if it was the union
class model:
    ...
    initial momentum field, through the given pair of images. Alternates
    shooting from either end of the path, rendering result more robust
    to switching the order of the inputs."""
    ...
    def _symmetrize(self, dc):
        ...
        uf_sym[-1] = dc.ub[-1]
        ub_sym[-1] = dc.uf[-1]
        for i in range(1, dc.params['timesteps'] - 1):
        ...
        for i in range(dc.params['timesteps']):
